...
Tổng số bảng: **16** (không tính materialized view).
...
**Quan hệ:** `diseases` 1-N với `disease_cases`, `feature_configs`, `model_versions`, `predictions`, `risk_thresholds`.
...
| `iso_year` | SMALLINT | - | Năm ISO |
| `iso_week` | SMALLINT | - | Tuần ISO |
| `feature_version` | VARCHAR(10) | - | Version feature set |
...
### Bảng 11 - `risk_thresholds`
**Mục đích:** ngưỡng phân loại Low/Medium/High theo quốc gia.

| Tên cột | Kiểu dữ liệu | PK/FK | Ý nghĩa |
|---|---|---|---|
| `id` | SERIAL/INTEGER | PK | Khóa chính |
| `disease_id` | INTEGER | FK → `diseases.id` | Bệnh |
| `iso3` | VARCHAR(10) | - | Quốc gia (hoặc `_global`) |
| `q33` | DOUBLE PRECISION | - | Ngưỡng Low/Medium |
| `q67` | DOUBLE PRECISION | - | Ngưỡng Medium/High |
| `n_nonzero_weeks` | INTEGER | - | Số tuần non-zero |
| `is_global_fallback` | BOOLEAN | - | Cờ fallback |
| `model_version_id` | INTEGER | FK → `model_versions.id` | Version model |
...
| `risk_q33` | DOUBLE PRECISION | - | Ngưỡng legacy |
| `risk_q67` | DOUBLE PRECISION | - | Ngưỡng legacy |
| `model_version_id` | INTEGER | FK → `model_versions.id` | Version model |
...
### Bảng 14 - `pipeline_run_logs`
**Mục đích:** log chi tiết từng step của pipeline.

| Tên cột | Kiểu dữ liệu | PK/FK | Ý nghĩa |
|---|---|---|---|
| `id` | BIGSERIAL | PK | Khóa chính |
| `run_id` | UUID | FK → `pipeline_runs.run_id` | Lần chạy pipeline |
| `step_name` | VARCHAR(100) | - | Tên bước |
| `level` | VARCHAR(10) | - | Mức log (INFO/WARNING/ERROR) |
| `message` | TEXT | - | Nội dung log |
| `logged_at` | TIMESTAMPTZ | - | Thời điểm log |

---

### Bảng 15 - `data_quality_checks`
...
| `client_ip` | INET | - | IP client |
| `requested_at` | TIMESTAMPTZ | - | Thời điểm gọi |
...
2. `diseases` 1-N `disease_cases`, `feature_configs`, `model_versions`, `predictions`, `risk_thresholds`.
3. `model_versions` 1-N `model_evaluations`, `predictions`, `risk_thresholds`.
4. `pipeline_runs` 1-N `pipeline_run_logs`, `data_quality_checks`.
...
5. **MLOps** → log `pipeline_runs`, `pipeline_run_logs`, kiểm tra `data_quality_checks`.
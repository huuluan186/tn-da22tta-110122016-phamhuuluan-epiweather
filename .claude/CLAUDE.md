...
## Quy tắc viết nội dung báo cáo / tài liệu (chống văn phong AI)

Áp dụng cho MỌI nội dung văn xuôi đưa vào báo cáo tốt nghiệp, slide, tài liệu docs/,
chương luận văn. Mục tiêu: văn bản đọc như sinh viên năm cuối tự viết sau khi đã làm
hệ thống, KHÔNG lộ dấu hiệu AI sinh. Đây là yêu cầu bắt buộc, không phải gợi ý.

### Dấu hiệu AI phải loại bỏ

1. **Cấu trúc bullet "nhãn: giải thích" lặp lại.** Không liệt kê đều tăm tắp
   `Khả năng mở rộng: ...` / `Tính bảo mật: ...` suốt nhiều đoạn.
   → Mặc định viết ĐOẠN VĂN. Chỉ dùng gạch đầu dòng cho dữ liệu rời rạc thật sự
   (danh sách 17 biến thời tiết, danh sách endpoint, bảng so sánh model).
2. **Tự ý in đậm từ khóa trong thân bài.** TUYỆT ĐỐI không bold "hiệu quả",
   "quan trọng", "tối ưu"... giữa câu. Chỉ bold khi định nghĩa thuật ngữ lần đầu
   hoặc tên biến/hàm/file.
3. **Câu chung chung, "nổ" hơn sự thật.** Cấm "đóng vai trò vô cùng quan trọng",
   "một cách toàn diện và hiệu quả". Mỗi câu phải nói được: cái gì, bao nhiêu,
   trong điều kiện nào (số liệu, tên cụ thể, nguồn trích).
4. **Cụm chuyển sáo rỗng.** Cắt ~80%: "bên cạnh đó", "hơn nữa", "đáng chú ý là",
   "có thể thấy rằng", "nhìn chung", "tóm lại", "nói cách khác". Nối câu bằng
   nội dung, không bằng từ nối trang trí.
5. **Luật bộ ba (rule of three).** Tránh "nhanh, chính xác và hiệu quả",
   "thu thập, xử lý và phân tích". Cắt còn 1-2 hoặc đổi nhịp.
6. **Câu kết đoạn kiểu khẩu hiệu/hướng tương lai.** Không khóa mỗi mục bằng
   "Điều này tạo nền tảng cho... ở chương sau." Kết bằng sự thật cụ thể hoặc
   một hạn chế.
7. **Ngôi "bạn" / giọng hội thoại.** Dùng ngôi vô nhân xưng: "đồ án sử dụng...",
   "phần này trình bày...", "mô hình được huấn luyện trên...". Không "bạn",
   "chúng ta nhé".
8. **Dấu nháy nhấn mạnh + em-dash rải rác.** Bỏ nháy 'nhấn mạnh'; dùng dấu câu
   bình thường.
9. **Hedging quá mức.** Có số liệu thì khẳng định; không chắc thì nêu rõ điều kiện,
   không hedge chung chung ("tương đối", "phần nào", "nhìn chung có xu hướng").
10. **Nhịp văn đều như máy.** Cố ý phá nhịp: xen câu ngắn (6-8 chữ) với câu dài.

### Quy tắc thay thế (chốt)

- Mặc định đoạn văn, không list (trừ dữ liệu rời rạc thật sự).
- Không bold trong thân bài.
- Mỗi đoạn mở bằng câu chủ đề có nội dung cụ thể, không mở bằng từ nối.
- Mỗi khẳng định kèm số liệu / tên cụ thể / nguồn (Bortman 1999, WHO EWARS, bảng X).
- Ngôi vô nhân xưng.
- Được phép và NÊN nêu hạn chế, đánh đổi, giả định — con người viết luận văn luôn
  thừa nhận giới hạn; AI hay tô hồng.
- Giữ thuật ngữ tiếng Anh đúng quy ước (training, overfitting, walk-forward CV...).
- Tiếng Việt có dấu đầy đủ (theo quy tắc đã có ở trên).
- Chỉ dùng đúng dữ kiện thật của project, không bịa số/feature/nguồn.

### Cách làm việc khi user nhờ viết nội dung báo cáo

- Viết theo TỪNG ĐOẠN dựa trên dữ kiện user cung cấp, không sinh một mạch cả chương
  rồi đưa nguyên khối (sự "đều" và "mượt" toàn văn là dấu hiệu AI rõ nhất).
- Sau khi viết, nhắc user tự sửa tay vài câu mỗi đoạn (thêm nhận xét riêng, con số
  nhớ được, khó khăn lúc làm) để tạo "vết tay" người viết.
- Nếu thiếu dữ kiện cụ thể, HỎI user thay vì điền bằng câu chung chung.
```